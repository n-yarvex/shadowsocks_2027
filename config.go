...
type AEAD struct{cipher.AEAD;Nonce[nonceSize]byte}
func NewAEAD(ctx,key []byte)*AEAD{k:=make([]byte,keySize);blake3.DeriveKey(k,string(ctx),key);aead,err:=chacha20poly1305.NewX(k);if err!=nil{panic(err)};return&AEAD{AEAD:aead}}
func(a*AEAD)Seal(dst,nonce,plaintext,additionalData []byte)[]byte{if nonce==nil{nonce=IncreaseNonce(a.Nonce[:])};return a.AEAD.Seal(dst,nonce,plaintext,additionalData)}
func(a*AEAD)Open(dst,nonce,ciphertext,additionalData []byte)([]byte,error){if nonce==nil{nonce=IncreaseNonce(a.Nonce[:])};return a.AEAD.Open(dst,nonce,ciphertext,additionalData)}
